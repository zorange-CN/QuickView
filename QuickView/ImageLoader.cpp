...
static void FindHeifGainMapManual(const uint8_t* data, size_t size, uint64_t* outOffset, uint64_t* outLength, uint32_t* outPitmOffset = nullptr, uint8_t* outPitmSize = nullptr, uint32_t* outBestID = nullptr) {
    ...
    *outOffset = 0; *outLength = 0;
    ...
    {
        QV_LOG("Scanner_AuxlCandidates",
            TraceLoggingInt32(auxlCount, "AuxlCount"),
    ...
    if (bestLen > 0) {
        ...
        *outLength = bestLen;
        ...
        QV_LOG("Scanner_Solved",
            TraceLoggingUInt32(bestID, "ItemID"),
...
static void ProbeHdrMetadataNative(const uint8_t* data, size_t size, QuickView::HdrStaticMetadata* pHdr) {
    ...
    for (size_t i = 0; i + urnLen <= urnScanLimit; ++i) {
        if (memcmp(data + i, appleUrn, urnLen) == 0) {
            ...
            QV_LOG("Metadata_HDR", TraceLoggingString("AppleGainMap Detected", "Action"));
            break;
        }
    }
}
...
HRESULT CImageLoader::FullDecodeToMMF(const uint8_t* data, size_t size,
    ...
    if (fmt == L"JXL") {
        ...
        for (;;) {
            ...
            if (st == JXL_DEC_BASIC_INFO) {
                ...
                if (needed > mmfBufSize) {
                    QV_LOG("MMF_Decode",
                        TraceLoggingString("JXL OOM", "Action"),
        ...
        *outStride = (int)(info.xsize * 4);
        ...
        QV_LOG("MMF_Decode",
            TraceLoggingString("JXL DirectToMMF OK", "Action"),
            TraceLoggingUInt32(info.xsize, "Width"),
            TraceLoggingUInt32(info.ysize, "Height"));
        return S_OK;
    }
    ...
    if (fmt == L"PNG") {
        ...
        *outStride = (int)((size_t)w * 4);
        ...
        QV_LOG("MMF_Decode",
            TraceLoggingString("PNG DirectToMMF OK", "Action"),
            TraceLoggingUInt32(w, "Width"),
            TraceLoggingUInt32(h, "Height"));
        return S_OK;
    }
    ...
    if (needed > mmfBufSize) {
        QV_LOG("MMF_Decode",
            TraceLoggingString("Fallback OOM", "Action"),
            TraceLoggingUInt64(needed / (1024*1024), "NeededMB"),
            TraceLoggingUInt64(mmfBufSize / (1024*1024), "MmfMB"));
        return E_OUTOFMEMORY;
    }
    ...
    *outStride = heapFrame.stride;
    ...
    QV_LOG("MMF_Decode",
        TraceLoggingString("FallbackCopy OK", "Action"),
                                           ...
                                           const uint8_t* mappedData, size_t mappedSize)
{
    ...
    for (;;) {
        ...
        JxlDecoderStatus st = JxlDecoderProcessInput(dec);
        if (st == JXL_DEC_ERROR) {
             QV_LOG("JXL_ROI", TraceLoggingString("DecoderError ProcessInput", "Action"));
             break;
        }
        if (st == JXL_DEC_SUCCESS) { hr = S_OK; break; }
        ...
        if (st == JXL_DEC_BASIC_INFO) {
            ...
            if (!BuildRegionScalePlan(srcRect, (int)info.xsize, (int)info.ysize, scale,
                                      explicitTargetW, explicitTargetH, &plan)) {
                QV_LOG("JXL_ROI", TraceLoggingString("BuildRegionScalePlan Failed", "Action"));
                hr = E_FAIL;
                break;
            }
            planReady = true;
            ...
            if (JXL_DEC_SUCCESS != JxlDecoderSetImageOutCallback(dec, &pixFmt, JxlCropCallback, &ctx)) {
                QV_LOG("JXL_ROI", TraceLoggingString("SetImageOutCallback Failed", "Action"));
                hr = E_FAIL; break;
            }
        }
        else if (st == JXL_DEC_FULL_IMAGE) {
            ...
            QV_LOG("JXL_ROI", TraceLoggingString("NeedMoreInput InputClosed", "Action"));
            break;
        }
    }
        ...
        namespace UltraHdr {
                ...
                const DecodeContext& ctx)
            {
                ...
                jerr.error_exit = [](j_common_ptr ci) {
                    QV_LOG("UltraHDR_Decode", TraceLoggingString("GainMap FatalError", "Action"));
                ...
                if (jpeg_read_header(&cinfo, TRUE) != JPEG_HEADER_OK) {
                    QV_LOG("UltraHDR_Decode", TraceLoggingString("ReadHeader Failed", "Action"));
                    jpeg_destroy_decompress(&cinfo);
                    return nullptr;
                }
        ...
        namespace JPEG {
            ...
            static HRESULT Load(const uint8_t* pBuf, size_t bufSize, const DecodeContext& ctx, DecodeResult& result) {
                ...
                // [CMS] Extract and Merge Multi-segment ICC Profile from APP2 markers
                {
                    ...
                    {
                        QV_LOG("Loader_JPEG",
                            TraceLoggingInt32(iccNumMarkers, "IccSegments"),
            ...
            static HRESULT Load(LPCWSTR filePath, const DecodeContext& ctx, DecodeResult& result) {
                ...
                if (openResult != LIBRAW_SUCCESS) {
                    QV_LOG("RawCodec_Load",
                        TraceLoggingString("OpenFailed", "Action"),
                ...
                if (!forceRawStart) {
                    ...
                    // Debug logging
                    QV_LOG("RawCodec_Load",
                        TraceLoggingString("UnpackThumb", "Action"),
                    ...
                    if (unpackResult == LIBRAW_SUCCESS) {
                        ...
                        QV_LOG("RawCodec_Load",
                            TraceLoggingString("ThumbInfo", "Action"),
                        ...
                        if (thumb) {
                            if (thumb->type == LIBRAW_IMAGE_JPEG) {
                                ...
                                if (SUCCEEDED(hr)) {
                                    ...
                                    QV_LOG("RawCodec_Load", TraceLoggingString("PreviewJPEG OK", "Action"));
                                    return S_OK;
                                }
                                QV_LOG("RawCodec_Load", TraceLoggingString("PreviewJPEG Failed", "Action"));
                            }
                            else if (thumb->type == LIBRAW_IMAGE_BITMAP) {
                                // RGB Bitmap
                                if (thumb->bits == 8 && thumb->colors == 3) {
                ...
                if (animator && animator->Initialize(fileMap, QuickView::PixelFormat::BGRA8888)) {
                    if (animator->IsAnimated()) {
                        ...
                        if (firstFrame && firstFrame->pixels) {
                            ...
                            QV_LOG("Anim_Loader", TraceLoggingString("Hijack Success", "Action"));
                            return S_OK;
                        } else {
                            QV_LOG("Anim_Loader", TraceLoggingString("GetNextFrame Null", "Action"));
                        }
                    // QV_LOG("Anim_Loader", TraceLoggingString("NotAnimated", "Action"));
                }
            } else {
                // QV_LOG("Anim_Loader", TraceLoggingString("InitializeFailed", "Action"));
            }
            } else {
                QV_LOG("Anim_Loader", TraceLoggingString("MappedFile Invalid", "Action"));
            }
        }
...
HRESULT CImageLoader::LoadThumbJXL_DC(const uint8_t* pFile, size_t fileSize, ThumbData* pData, ImageMetadata* pMetadata, bool allowFakeBase) {
    ...
    for (;;) {
        ...
        if (status == JXL_DEC_ERROR) {
            ...
            if (!foundDC && info.have_preview) {
                ...
                if (previewSize > 0 && outSize == previewSize) {
                    ...
                    if (previewSize <= kPreviewCap) {
                        ...
                        QV_LOG("JXL_DC",
                            TraceLoggingString("PreviewFallback", "Action"),
            ...
            {
                QV_LOG("JXL_DC",
                    TraceLoggingString("No18DC", "Action"),
